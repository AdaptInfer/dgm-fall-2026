...
Stacking perceptrons allows a neural network to represent functions such as XOR, but the network needs a way to learn the appropriate weights. Backpropagation uses the chain rule to calculate gradients and update those weights.
These gradients are then used to update the network's parameters during training.
